        ...
        for (int i1=0; i1<inputs.length; i1++){
            ...
            if (isStart==1 & ab_sum==0){
                for (int i2=a_index.size()-1; i2>=0; i2--){
                    ...
                    right = str_in.substring(b_index.get(i2)-right_index_fix, str_in.length());
                    middle = str_in.substring(a_index.get(i2)+1, b_index.get(i2)-right_index_fix);
                    middle_temp = "";
                    for (int i3 = 0; i3<(int)inputs[a_index.get(i2) - 1]; i3++){
                        middle_temp = middle_temp.concat(middle);
                    }
                    str_in = left + middle_temp + right;
                    right_index_fix = (b_index.get(i2)-right_index_fix - a_index.get(i2) + 2) - ((int)inputs[a_index.get(i2) - 1])*(b_index.get(i2)-right_index_fix - a_index.get(i2) - 1);
                }
            }
        }
        return str_in;
